...
def qpot(x,p,r,w):
    """
    basis expotential function exp(-r/re)     
    """
    ...
    S = np.zeros((Nb,Nb))
    f = []   
    f.append(np.array([1.0]*len(x)))  
    f.append(np.exp(-a * x))
    f.append(np.exp(-2.0 * a * x))
    
    df = [] 
    df.append(np.zeros(len(x))) 
    df.append(-a * np.exp(-a*x))
    df.append(-2. * a * np.exp(-2.0 * a * x))
    
    for m in range(Nb):
        for n in range(Nb):
            S[m,n] = np.dot(f[m]*f[n],w)
    ...
    ddr = a**2 * cr[1] * f[1]  + 4.0 * a**2 * cr[2] * f[2] 
    ddp = a**2 * cp[1] * f[1]  + 4.0 * a**2 * cp[2] * f[2] 
