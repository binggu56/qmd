...
#   dat = np.genfromtxt('dat4/xAve.dat')
#    ax.plot(dat[:,0],dat[:,1],'-',label='nfit=3')
    
#   dat = np.genfromtxt('dat11/xAve.dat')
#   ax.plot(dat[:,0],dat[:,1],'-',label='cubic')
    
    dat = np.genfromtxt('/home/bingg/pyQMD/SPO/spo_2d/1.0.3/dat2/xave.dat')     
...
#dat = np.genfromtxt(fname='dat4/den.dat',dtype=np.complex128)
    #ax.plot(dat[:,0], dat[:,1].real)
#   ax.plot(dat[:,0], dat[:,1].imag,'--')

    # exact results

    dat = np.genfromtxt('/home/bingg/pyQMD/SPO/spo_2d/1.0.3/dat2/den.dat')
    #ax.plot(dat[:,0], dat[:,1],label='real, QM')
    ax.plot(dat[:,0], dat[:,2],'k',lw=2)

    # results without coupling
    x = np.linspace(0,20,200)
    ax.plot(x,np.exp(-0.5) / 2.0 * np.sin(x),'--',linewidth=1)
