...
ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
	...
	real*8 ak2(ni,nj),mx,my,norm,akx(ni,nj),aky(ni,nj)
	complex*16 v(ni,nj)
...
C Henon-Hails
	...
	v(i,j)=wx**2*x**2/2d0+wy**2*y**2/2d0+(x**2*y-y**3/3d0)*corr
...
c-------------------------------------------------------
	...
	integer*4,intent(IN)::ni,nj
        ...
        dimension nn(2)
        complex*16:: psi(ni,nj),v(ni,nj),po,ki
        complex*16 psi0(ni,nj),hpsi(ni,nj),c1,c2
	common /grid/ xmin,ymin,xmax,ymax,dx,dy
...
C	
	...
	write(20,1000) t,en0,enx,eny,po,ki,ex
	write(15,1000) t,c1,abs(c1)
	write(16,1000) 2d0*t,c2,abs(c2)
