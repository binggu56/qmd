     ...
     &                      ddv(:),s(:)
      complex*16,allocatable :: h(:,:),mat(:,:),psib(:)
     	real*8 :: f(3),ki
      complex*16,allocatable :: c(:),c0(:),dc(:)
      real :: gasdev
      complex*16 :: psi,z0,zn
...
1002  format(/'1D QTM code with Gassian Basis '//,
     ...
     & u(Ntraj),dv(Ntraj),ddv(Ntraj))
      ...
      allocate(c(Ntraj),c0(Ntraj),h(Ntraj,Ntraj),dc(Ntraj),psib(Ntraj))     
...
!------check normalization--------------
      ...
      do i=1,Np
        ...
        psi = (0d0,0d0)
        do j=1,nb
          psi = psi+c0(j)*exp(-al/2d0*(xi-x(j))**2+im*p(j)*(xi-x(j)))*
     +          dsqrt(dsqrt(al/pi))
        enddo
        prob = prob+abs(psi)**2*dx
        if(abs(psi) .gt. 1d-3) write(104,10000) xi,abs(psi)**2
      enddo
      write(*,1013)  prob
...
!         propogate trajectories with Erenfest theorem
!         -------------------------------
!          call derivs(x,Ntraj,v,dv,ddv)
!          call qpot(Nb,ddv,c,am,p,x,w,u,du)
...
!       print out trajectories in phase space
        ...
        write(105,10000) t,(p(i),i=1,Ntraj)
        write(106,10000) t,(x(i),i=1,Ntraj)
...
!     print out final wavefunction         
      ...
      do i=1,Np
        ...
        psi = (0d0,0d0)
        do j=1,nb
          psi = psi+c(j)*exp(-al/2d0*(xi-x(j))**2+im*p(j)*(xi-x(j)))*
     +          dsqrt(dsqrt(al/pi))
        enddo
        prob = prob+abs(psi)**2*dx
        if(abs(psi) .gt. 1d-3) write(103,10000) xi,abs(psi)**2
      enddo
