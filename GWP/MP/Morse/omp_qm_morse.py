...
def omp(aold, xold, pold, cold):
    ...
    #nmax = 100 # maximum number of basis defined by user 
...
N = 200     # number of elements in the dictionary 
#gx = np.random.randn(N) / np.sqrt(2.0 * a0) + x0 
gx = np.linspace(0.4,2.3,N)
gp = np.zeros(N)
#gp = np.random.randn(N)
#gp += p0
a = 32.0 # basis of GWPs width 
...
for k in range(Nt):
    ...
    if cnum > 1e5: 
        ...
        pold = p 
        cold = c 
        x,p,c,index = omp(a, xold,pold,cold)
