...
def plot(ax1):
    ...
    ax1.plot(x,psi**2,linewidth=2,label='Psi')
    ax1.plot(x,psi_approx**2,'--',linewidth=2,label='Fit')
    
    x, exact = np.genfromtxt(fname='/home/bing/gwp/spo/1.0.3/wft.dat',unpack=True) 
    
    ax1.plot(x, exact, linewidth=2,label='Exact')
    #ax1.legend() 
