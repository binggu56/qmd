...
compute p from the  approximated wavefunction 

use position basis as test function 
...
def dirac_overlap(x): 
    ...
    for j in range(N):
        ...
        for k in range(N):
            ...
            S[j,k] = (a/np.pi)**0.25 * np.exp(-a/2.0 * dq**2)
...
def get_p(xgrid, x,p,c, smooth=True):
    """
...
def vmat_FGA(x):
    ...
    for j in range(Ntraj):
        ...
        for k in range(Ntraj):
            ...
            l[j,k] = v0 * (a/np.pi)**0.25 * np.exp(-a/2.0 * dx**2)
...
def kmat_FGA(x):
    ...
    for j in range(Ntraj):
        ...
        for k in range(Ntraj):
            ...
            dx = x[j] - x[k]            
            l[j,k] = (a**2*dx**2 - a) * (a/np.pi)**0.25 * np.exp(-a/2.0 * dx**2)
...
def dmat_FGA(x,p):
    """
    time-derivative matrix D = <gj| d/dt | gk> = <gj| -dp/(2m)*ak*(x-qk)^2 + ak*pk/m * (x-qk) - dp/(4m) | gk>
    the trajectory momentum p is different from the momentum for basis (gp)    
    """
    Ntraj = len(x)
    ...
    for j in range(Ntraj): 
        ...
        for k in range(Ntraj):
            ...
            l[j,k] = a * p[k] * dx * (a/np.pi)**0.25 * np.exp(-a/2.0 * dx**2)
...
# define the initial wavefunction, GWP 
...
for k in range(Nt):
    ...
    # renormalization 
    anm = np.vdot(c,overlap(x,p).dot(c)).real    
    c = c/np.sqrt(anm)
