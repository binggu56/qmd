...
N = 200 # number of elements in dictionary  
...
res = 1.0 - sum(np.array(np.abs(c))**2)
    
while res > 1e-4 and len(x) < nmax: 
    
    d = [gint(gx[k],gp[k],a, x[-1], p[-1], a) for k in range(N)]
    tmp = tmp - c[-1] * np.array(d)  
