...
!      complex*16, dimension(nmax) :: c,c0,dc 
      ...
      complex*16 :: z0,z1,z2,zn,z,corr,cor 
...
! --- grid to sample initial points      
      xl = qx0-1.d0*dsqrt(pow/ax0)
      xr = qx0+1.d0*dsqrt(pow/ax0)
      yl = qy0-1.d0*dsqrt(pow/ay0)
      yr = qy0+1.d0*dsqrt(pow/ay0)
...
!        do j=1,nby
!          k = nby*(i-1)+j
10        x(i) = (2d0*ranf()-1d0)*(xr-xl)+qx0
!          x(i) = x(i)*(xr-xl)+qx0
!      enddo 
!      do i=1,nb
          y(i) = (2d0*ranf()-1d0)*(yr-yl)+qy0
!          y(i) = y(i)*(yr-yl)+qy0 
!        if( i > 1) then 
        do j=1,i-1
          dx0 = x(i) - x(j)
          dy0 = y(i) - y(j) 
          d0 = ax*dx0*dx0/4d0+ay*dy0*dy0/4d0
          if(d0 < cut) then 
            goto 10 
          endif 
        enddo 
...
! ----- correlation function 
        ...
        cor = corr(nb,c0,c,x,y,px,py,s)
        write(115,1000) t,cor,abs(cor) 
...
!        write(101,1000) (x(i),i=1,nb),(p(i),i=1,nb) 
        ...
        write(105,1000) t,(px(i),i=1,Nb,10)
        write(106,1000) t,(x(i),i=1,Nb,10)
...
! c=2**63-1.  Here the seed is a global variable.
!
      USE CSEED
      IMPLICIT NONE
      INTEGER*8 :: H, L, T, A, C, Q, R
!      DATA A/16807/, C/9223372036854775807/, Q/548781581296767/, R/12838/
...
! ---- autocorrelation using symmetrized wavefunction --------
      ...
      subroutine permute(nb,x,y,px,py,c,s) 
...
!---- compute c0_k = <psi0|g_k>
      do j=1,nb
        ...
        dpx = px0+im*ax*dx  
        dpy = py0+im*ay*dy 
        z0(j) = -0.5d0*(dpx**2/asx+dpy**2/asy) -0.5d0*(ax*dx*dx+ay*dy**2)
!        d0=-0.5d0/as*dp*dp
!        d1=(a*p0)/as*dq
...
!---- compute c0_k = <permuted psi0|g_k>
      do j=1,nb
        ...
        dpx = py0+im*ax*dx  
        dpy = px0+im*ay*dy 
        z1(j) = -0.5d0*(dpx**2/asx+dpy**2/asy) -0.5d0*(ax*dx*dx+ay*dy**2)
!        d0=-0.5d0/as*dp*dp
!        d1=(a*p0)/as*dq
!        c0(j)=exp(d2+d0+im*d1)*an
        z1(j) = an*exp(z1(j))
...
! ---- compute < psi0 | permuted gk > 
      ...
      do j=1,nb
        ...
        dpx = py0+im*ax*dx  
        dpy = px0+im*ay*dy 
        z2(j) = -0.5d0*(dpx**2/asx+dpy**2/asy) -0.5d0*(ax*dx*dx+ay*dy**2)
!        d0=-0.5d0/as*dp*dp
!        d1=(a*p0)/as*dq
!        c0(j)=exp(d2+d0+im*d1)*an
        z2(j) = an*exp(z2(j))
...
! ---- compute < permuted psi0 | permuted gk > 

      corr = (0d0,0d0)
      do j=1,nb 
          corr = corr + z1(j)*exp(im*s(j))*c(j)
      enddo 
...
!        v11 = 1d0+3d0*ep*x**2 
!        v22 = 1d0 
      v0 = -2d0/abs(x)-2d0/abs(y)+1d0/abs(x+y) 
...
!          z = dq/2d0
          ...
          d0 = ax/(4d0*amx)*(1d0-ax/2d0*dx**2)+ay/(4d0*amy)*(1d0-ay/2d0*dy**2)
          xc = (x(j)+x(k))/2d0 
          yc = 0.5*(y(j)+y(k))
!          call pot(xc,yc,v0,vxx,vyy)
!          d1 = v0 + vxx/(4d0*ax) + vyy/(4d0*ay)
