...
! ----------------------------------------------------------------------
      ...
      integer (kind = 4) :: nd ! number of domains  
      
!      integer (kind = 4), parameter :: 

      real (kind = 8), dimension(100) ::  ydom
...
! --- density overlap 
      ...
      do i=1,ntraj
        if(x(2,i) > 0d0) then  
        d0 = d0 + w(i)*x(1,i)**2*exp(-beta*x(1,i)**2)
        d1 = d1 + w(i)*exp(-beta*x(1,i)**2)
       endif 
      enddo 
      d0 = d0*(2d0*beta)*dsqrt(beta/pi) 
      d1 = d1*dsqrt(beta/pi) 
      

      write(103,1000) t,d1,d0 
