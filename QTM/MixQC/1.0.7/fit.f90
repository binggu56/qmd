...
!      call basis(Ndim,Ntr,x,f)
      ...
      l = 1 
      do while(l .le. nd) ! loop over domains    
...
! --- domain function depending on the domain index l 
      ...
      do i=1,ntraj 
        dom(i) = 0.5d0*(tanh(aka*(y(i)-ydom(l)))-tanh(aka*(y(i)-ydom(l+1))))
        ddom(i) = aka*0.5d0*(-tanh(aka*(y(i)-ydom(l)))**2 + tanh(aka*(y(i)-ydom(l+1)))**2)
      enddo     
